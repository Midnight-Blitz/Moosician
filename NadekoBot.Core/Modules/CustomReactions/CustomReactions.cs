...
namespace NadekoBot.Modules.CustomReactions
{
    public class CustomReactions : NadekoTopLevelModule<CustomReactionsService>
    {
        ...
        public async Task AddCustReact(string key, [Remainder] string message)
        {
            ...
            if ((channel == null && !_creds.IsOwner(Context.User)) || (channel != null && !((IGuildUser)Context.User).GuildPermissions.ManageEmojis))
            {
        ...
        {
            if ((Context.Guild == null && !_creds.IsOwner(Context.User)) || (Context.Guild != null && !((IGuildUser)Context.User).GuildPermissions.ManageEmojis))
            {
                await ReplyErrorLocalized("insuff_perms").ConfigureAwait(false);
                return;
            }
        ...
        public async Task CrCa(int id)
        {
            if ((Context.Guild == null && !_creds.IsOwner(Context.User)) ||
                (Context.Guild != null && !((IGuildUser)Context.User).GuildPermissions.ManageEmojis))
            {
                await ReplyErrorLocalized("insuff_perms").ConfigureAwait(false);
                return;
            }
        ...
        public async Task CrClear()
        {
            if (await PromptUserConfirmAsync(new EmbedBuilder()
                ...
                .WithDescription("This will delete all custom reactions!!!")).ConfigureAwait(false))
            {
