    ...
    public partial class Utility
    {
        ...
        public class QuoteCommands : NadekoSubmodule
        {
            ...
            public async Task ShowQuote([Remainder] string keyword)
            {
                ...
                {
                    rep.Replace(crembed);
                    await Context.Channel.EmbedAsync(crembed.ToEmbed(), $"*️⃣`{quote.Id}`" + crembed.PlainText?.SanitizeMentions() ?? "")
                        .ConfigureAwait(false);
                    return;
                }
                await Context.Channel.SendMessageAsync($"*️⃣`{quote.Id}`" + rep.Replace(quote.Text)?.SanitizeMentions()).ConfigureAwait(false);
            }
            ...
            public async Task QuoteSearch(string keyword, [Remainder] string text)
            {
                ...
                await Context.Channel.SendMessageAsync($"*️⃣`{keywordquote.Id}`" + keyword.ToLowerInvariant() + ":  " +
            ...
            {
                ///editvars
                var isEmojiMaster = ((IGuildUser)Context.Message.Author).GuildPermissions.ManageEmojis;
                var isMod = ((IGuildUser)Context.Message.Author).GuildPermissions.ManageRoles;
                ...
                using (var uow = _db.UnitOfWork)
                {
                    ...
                    if ((q?.GuildId != Context.Guild.Id) || (!isEmojiMaster && q.AuthorId != Context.Message.Author.Id))
                    {
