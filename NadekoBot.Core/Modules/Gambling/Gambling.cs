...
namespace NadekoBot.Modules.Gambling
{
    public partial class Gambling : GamblingTopLevelModule<GamblingService>
    {
        ...
        public async Task Timely()
        {
            ...
            await ReplyConfirmLocalizedAsync("timely", val + Bc.BotConfig.CurrencySign, period).ConfigureAwait(false);
        }
        ...
        public async Task TimelySet(int num, int period = 24)
        {
            ...
            if (num == 0)
                ...
                await ReplyConfirmLocalizedAsync("timely_set", Format.Bold(num + Bc.BotConfig.CurrencySign), Format.Bold(period.ToString())).ConfigureAwait(false);
        }
        ...
        public async Task Give(ShmartNumber amount, IGuildUser receiver, [Remainder] string msg = null)
        {
            ...
            await ReplyConfirmLocalizedAsync("gifted", amount + CurrencySign, Format.Bold(receiver.ToString()), msg)
        ...
        public async Task Take(ShmartNumber amount, [Remainder] ulong usrId)
        {
            ...
            if (await _cs.RemoveAsync(usrId, $"Taken by the fun police.({Context.User.Username}/{Context.User.Id})", amount,
                gamble: (Context.Client.CurrentUser.Id != usrId)).ConfigureAwait(false))
