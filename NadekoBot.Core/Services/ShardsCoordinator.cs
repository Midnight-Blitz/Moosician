...
namespace NadekoBot.Core.Services
{
    public class ShardsCoordinator
    {
        ...
        public ShardsCoordinator()
        {
            ...
            _log.Info("Starting Moosician v" + StatsService.BotVersion);
            ...
            var _ = Task.Run(async () =>
            {
                do
                {
                    ...
                    while (_shardStartQueue.TryPeek(out var id))
                    {
                        ...
                        {
                            _log.Warn("Auto-restarting shard {0}, {1} in queue.", id, _shardStartQueue.Count);
                        }
                        else
                        {
                            _log.Warn("Starting shard {0}, {1} in queue.", id, _shardStartQueue.Count - 1);
                        }
